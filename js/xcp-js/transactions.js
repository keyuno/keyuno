...
function destroyXCP_opreturn(add_from, add_to, asset, asset_total, transfee, mnemonic) {
...
function sendBroadcast_opreturn_blockchaininfo(add_from, message, value, feefraction, transfee, mnemonic) {
    hex_byte();    
    ...
    $.getJSON( source_html, function( data ) {
        ...
        if (datachunk_unencoded != "error") {
			...
			if (final_trans != "errpr" && buildTxTime  && pushTxOK == 0) {
				sendBroadcastOK = 1;
...
function sendXCP_opreturn_blockchaininfo(add_from, add_to, asset, asset_total, btc_total, transfee, mnemonic) {
    ...
    $.getJSON( source_html, function( data ) {
		...
		var buildTxEnd = new Date().getTime();
		var buildTxTime = Number(buildTxEnd - buildTxStart);
        sendBTCpush_blockchaininfo1(final_trans);  //uncomment to push raw tx to the bitcoin network
